...
static Word32 Pitch_fr( Word32 *T0_prev_subframe, enum Mode mode, Word32 T_op[],
      ...
      , Word16 *resu3, Word32 *ana_index, int em_rate,int total_subframes,int filenu,int *change_frames,int frames,int *ste,int subfrNr,int m1,int m2,int m3)
{
   Float32 corr_v[40];
   Float32 max;
   Float32 *corr;
   ...
   int msgLength = em_rate*total_subframes*3/4/100;//每帧3bit
   ...
   //生成秘密信息
    if(ste[frames-1]==1&& subfrNr%2==1)//即第2第4子帧进行嵌入
   {
		
		//printf("ste:%d,修改第%d的%d子帧(%d),m1~m3:%d %d %d,lag修改前:%d,", ste[frames - 1], frames,subfrNr+1, js+1 , m1, m2, m3,lag);
		if (subfrNr == 1)//第2子帧不做限制
		{
			if ((lag % 2) != (m1))//整数基音延迟%2(即二进制最后一位)不等于秘密信息的奇偶性(0,1),则修改整数基音延迟
			{
			...
			l0_tmp = (lag % 4) / 2;
			//printf("l0_tmp:%d ", l0_tmp);
		}
		//第4子帧要同时满足奇偶性=m2和m3=((lag_1 % 4) / 2)^(lag_3 % 4) / 2)
		else if (subfrNr == 3)
		{
			do {
				if ((lag % 2) != (m2))//整数基音延迟%2(即二进制最后一位)不等于秘密信息的奇偶性(0,1),则修改整数基音延迟
				{
					if (lag <= 17)lag++;//小于17就+1
					else if (lag >= 143)lag--;//大于143就-1
					else //处于中间
				...
				l3_tmp = (lag % 4) / 2;
				//printf("l3_tmp:%d ", l3_tmp);
			} while (m3==l3_tmp^ l0_tmp);
		}
		else
			printf("error");
		//printf("LAG修改后:%d\n", lag);
		...
		//printf("ste:%d,不改第%d的%d子帧(%d),change_frames:%d=msgLength:%d,LAG:%d\n", ste[frames - 1], frames, subfrNr + 1, js+1, *change_frames, msgLength, lag);
...
static void cl_ltp( Word32 *T0_prev_subframe, Float32 *gp, enum Mode mode,
      ...
      *T0, Word32 *T0_frac, Float32 *gain_pit, Float32 gCoeff[], Word16 **anap,
      Float32 *gp_limit, int em_rate,int total_subframes,int filenu,int *change_frames,int frames,int *ste,int subfrNr,int m1,int m2,int m3)
{
   Float32 s;
   Word32 i, n;
   ...
   *T0 = Pitch_fr( T0_prev_subframe, mode, T_op, exc, xn, h1, frame_offset,
         T0_frac, &resu3, &i, em_rate,total_subframes, filenu, change_frames, frames, ste, subfrNr,m1,m2,m3);
   *( *anap )++ = ( Word16 )i;
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[], int em_rate,int total_subframes,int filenu,int *change_frames, int frames,int *ste)
{
   /* LPC coefficients */
   ...
   if (ste[frames-1]==1&& (em_rate != 0))//判断接下来这帧是否用于嵌入
   {
	   *change_frames = *change_frames + 1;
	   m1 = rand() % 2;
	   m2 = rand() % 2;
	   m3 = rand() % 2;
   }
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      cl_ltp( &st->clLtpSt->pitchSt->T0_prev_subframe, st->tonStabSt->gp, *
            ...
            lsp_flag, xn2, y1, &T0, &T0_frac, &gain_pit, gCoeff, &ana, &gp_limit, em_rate,total_subframes,filenu, change_frames, frames, ste, 
		  subfrNr,m1,m2,m3);
	  ////printf("%d ",T0_frac);
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode, int em_rate,int total_subframes,int filenu,int *change_frames, int frames, int *ste)
{
   ...
   cod_amr( state->cod_amr_state, mode, speech, prm, used_mode, syn, em_rate,total_subframes,filenu, change_frames, frames, ste);
