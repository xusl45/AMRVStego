...
FILE *Lag;
FILE *LagF;
//整数只有-2~3即110 101 000 001 010 011,无111和100.无法用三位的小数直接和秘密信息进行操作,只能第一符号位保留,剩下两位去和秘密信息操作,所以n=2.

int Binary_idx(int num, int j)
{
	int flag;//判断符号
	if (num < 0)
		flag = 1;
...
int relagf(int *a, int *b, int index)
{
	int flag, num = 0;
	//cout << b[0 + 3 * index]<< a[0 + 2 * index] << a[1 + 2 * index];
	flag = b[0 + 3 * index];
	num = 2 * a[0 + 2 * index] + 1 * a[1 + 2 * index];
	if (flag == 1)
		num = -num;
	//cout <<":" <<num << endl;
	return num;
}
//n=2,g1=0,g2=0时,都进行替换
//n=2,g1=0,g2=1时,有一个一样的就替换,没有就不替换
int PSV(int a[], int b[], int len)//求两个序列相同元素的个数
{
	int cc = 0;
	for (int i = 0; i < len; i++)
	{
...
static Word32 Pitch_fr( Word32 *T0_prev_subframe, enum Mode mode, Word32 T_op[],
      ...
      , Word16 *resu3, Word32 *ana_index, int em_rate, int total_subframes,int filenu,int *change_subframes, int frames, int subfrNr,int *ste,int* Lag,int *S,int g1,int g2)
{
   Float32 corr_v[40];
   Float32 max;
   Float32 *corr;
   ...
   Word16 delta_search;
   if(em_rate>0)
	//printf("第%d的%d(%d)子帧:嵌入标志:%d\n",frames, subfrNr+1,js+1,ste[js]);
   ...
   //以下代码修改frac
   if (em_rate>0&&ste[js] == 1)//g:00 01 02 12
   {
	   *change_subframes = *change_subframes + 1;
	   ...
	   int m1_c = m1^ s1;//加密
	   int m2_c = m2 ^s2;//加密
	   //printf("----------\n执行嵌入,m:%d%d,s:%d%d,s3:%d,加密后%d%d", m1, m2, s1, s2,s3, m1_c, m2_c);
	   //-2:110,-1:101
	   int sign;//保存符号
	   if (frac < 0)
		   sign = 1;
	   ...
	   int b1 =abs(frac)%2 ;
	   int b2 = abs(frac)/2;
	   //printf("\nfrac修改前%d,转为2进制:%d%d%d", frac,sign,b1,b2);
	   int psv=0;//相似度
	   if (m1_c == b1)psv++;
	   if (m2_c == b2)psv++;
	   if (psv >= g2)//阈值达到g2直接替换
	   {
		   b1 = m1_c; b2 = m2_c;
	   }
	   else if (psv >= g1 && psv < g2)//阈值处于中间用s3判断是否替换
	   {
		   if (s3 == 1)
		   {
			   b1 = m1_c; b2 = m2_c;
	   ...
	   int frac_tmp = 2*b1+1*b2;
	   if (sign == 1)
		   frac = -frac_tmp;
	   else
		   frac = frac_tmp;
	   //printf("相似度%d,g1:%d,g2:%d,frac修改后%d\n----------------\n",psv, g1,g2,frac);
   }
   if(em_rate>0)
	lag = Lag[js];//覆盖cover的LAG
...
static void cl_ltp( Word32 *T0_prev_subframe, Float32 *gp, enum Mode mode,
      ...
      *T0, Word32 *T0_frac, Float32 *gain_pit, Float32 gCoeff[], Word16 **anap,
      Float32 *gp_limit, int em_rate, int total_subframes,int filenu,int *change_subframes, int frames,int subfrNr, int *ste,int* Lag,int *S,int g1,int g2)
{
   Float32 s;
   Word32 i, n;
   ...
   *T0 = Pitch_fr( T0_prev_subframe, mode, T_op, exc, xn, h1, frame_offset,
         T0_frac, &resu3, &i, em_rate, total_subframes,filenu,change_subframes,frames, subfrNr, ste, Lag,S,g1,g2);
   *( *anap )++ = ( Word16 )i;
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
	  ...
	  int em_rate, int total_subframes,int filenu,int *change_subframes, int frames, int *ste,int* Lag,int *S,int g1,int g2)
{
   /* LPC coefficients */
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
	  ...
	  cl_ltp(&st->clLtpSt->pitchSt->T0_prev_subframe, st->tonStabSt->gp, *
		  ...
		  em_rate, total_subframes, filenu, change_subframes, frames, subfrNr, ste, Lag,S,g1,g2);
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode, int em_rate,int total_subframes,int filenu,int *change_subframes, int frames, int *ste,int* Lag,int *S,int g1,int g2)
{
