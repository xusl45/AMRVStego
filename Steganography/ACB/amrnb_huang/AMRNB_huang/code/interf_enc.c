...
int Encoder_Interface_Encode( void *st, enum Mode mode, Word16 *speech,
      ...
      int force_speech, int em_rate,int total_subframes,int filenu,int *change_subframes, int printsecret,int printpra)
{
   ...
   if (noHoming){
      Speech_Encode_Frame( s->encoderState, mode, speech, prm, &used_mode, em_rate,total_subframes,filenu, change_subframes, printsecret, printpra);
   }
   else {
      switch ( mode ) {
         case MR122:
