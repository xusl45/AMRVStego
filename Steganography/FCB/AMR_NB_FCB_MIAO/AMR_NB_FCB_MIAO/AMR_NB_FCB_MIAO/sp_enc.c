...
void searchSpace(Word32 pulse,Word32 *space,Word8 msg,int enta)
{
	Word32 temp;
	//msg=1;
	...
	/*space[0]=dgray[(gray[pulse/5]^msg)]*5+(pulse%5);
	space[1]=dgray[(gray[pulse/5]^(msg+4))]*5+(pulse%5);*/

	//printf("%d ",msg);
	//printf("msg: %d	enta=%d		pulse=%d	gray[pulse/5]=%d	\n",msg,enta,pulse,gray[pulse/5]);
	/*for(i=0;i<8;i++)
		printf("gray[%d]=%d\n",i,gray[i]);*/
	...
	//for(i=0;i<cases;i++)
	//{
	//	space[i]=dgray[(msg^gray[pulse/5])&itemp+etemp[i]]*5+(pulse%5);
	//	//printf("space[%d]=%d\n",i,space[i]);	
	//}
	...
	if (enta == 1) {
		ttemp = msg^enta;//(a + b) & 1;
		result = ttemp - gray[pulse/5] & 1;
		if (result < 0) {
			/*printf( "case 1:%d\n" ,result + 2);
			printf( "case 2:%d\n" , result + 4);
			printf( "case 3:%d\n" ,result + 6);
			...
			space[0] = dgray[(result + 2)]*5+(pulse%5);
			space[1] = dgray[(result + 4)]*5+(pulse%5);
			space[2] = dgray[(result + 6)]*5+(pulse%5);
			space[3] = dgray[(result + 8)]*5+(pulse%5);
		} else {
			/*printf( "case 1:%d\n" ,result);
			printf( "case 2:%d\n" , result + 2);
			printf( "case 3:%d\n" ,result + 4);
			...
			space[0] = dgray[(result)]*5+(pulse%5);
			space[1] = dgray[(result + 2)]*5+(pulse%5);
			space[2] = dgray[(result + 4)]*5+(pulse%5);
			space[3] = dgray[(result + 6)]*5+(pulse%5);
		}
	} else if (enta == 2) {
		ttemp =  msg^enta;//(a + b) & 3;
		result = ttemp - gray[pulse/5] & 3;
		if (result < 0) {
			/*printf( "case 1:%d\n" ,result + 4);
			printf( "case 2:%d\n" , result + 8);*/

			space[0] = dgray[(result + 4)]*5+(pulse%5);
			space[1] = dgray[(result + 8)]*5+(pulse%5);
		} else {
			/*printf( "case 1:%d\n" ,result );
			printf( "case 2:%d\n" , result + 4);*/
			space[0] = dgray[(result)]*5+(pulse%5);
		...
		ttemp =  msg^enta;//(a + b) & 7;
		result = ttemp - gray[pulse/5] & 7;
		if (result < 0)
		{
			//printf( "case 1:%d\n" ,result + 8);
			space[0] = dgray[(result + 8)]*5+(pulse%5);
		}
		else 
		{
			//printf( "case 1:%d\n" ,result);
...
Word32 getMsg(int pulse0,int pulse1,int enta)
{
	Word32 msg;
	int itemp;
	...
	//printf("\n**%d	%d	%d	%d**\n",pulse0,pulse1,gray[pulse0/5],gray[pulse1/5]);
	msg=(gray[pulse0/5]+gray[pulse1/5])&itemp^enta;
	//msg=(gray[pulse0/5] ^ gray[pulse1/5])%4;
...
static void search_10i40( Float32 dn[], Float32 rr[][L_CODE], Word32 ipos[],
      Word32 pos_max[], Word32 codvec[], int em_rate ,int enta)
{
   Float32 rrv[L_CODE];
   Float32 psk, ps, ps0, ps1, ps2, sq, sq2, alpk, alp, alp0, alp1, alp2;
   Float32 *p_r, *p_r0, *p_r1, *p_r2, *p_r3, *p_r4, *p_r5, *p_r6, *p_r7, *p_r8,
         *p_r9, *p_r10;
   Float32 *p_rrv, *p_rrv0, *p_dn, *p_dn0, *p_dn1, *p_dn_max;
   Word32 i0, i1, i2, i3, i4, i5, i6, i7, i8, i9, j, k, ia, ib, i, pos, space0[4], space1[4];
   int t,mark[5],m0,m1,r_emrate,r_msg,ten_em[5];
   ...
   for ( i = 1; i < 5; i++ ) {
	  ...
	  if(mark[i2%5]!=-1)
	  {
		  r_emrate = ten_em[i2%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));//产生随机的秘密信息
		      m0=1;
		      searchSpace(mark[i2%5],space0,r_msg,enta);
		  }
	  }
      if(mark[i3%5]!=-1)
	  {
		   r_emrate = ten_em[i3%5];
		   if(r_emrate < em_rate)
	       {
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m1=1;
			  searchSpace(mark[i3%5],space1,r_msg,enta);
		   }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r3 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
      ...
      do {
         ...
         do {
			...
			if(m0 == 0 && m1 == 0)
			{
				if ( ( alp * sq2 ) > ( sq * alp2 ) ) {
				   ...
				   ib = ( Word16 )( p_dn - dn );
					 ...
					 (
						(m0 == 1 && ((p_dn0 - dn) == space0[0] || (p_dn0 - dn) == space0[1]|| (p_dn0 - dn) == space0[2]|| (p_dn0 - dn) == space0[3])) 
						&& 
						(m1 == 1 && ((p_dn  - dn) == space1[0] || (p_dn  - dn) == space1[1]|| (p_dn - dn) == space1[2]|| (p_dn - dn) == space1[3]))
					 )
					 ||
					 (
						(m0 == 0) && (m1 == 1 && ((p_dn  - dn) == space1[0] || (p_dn  - dn) == space1[1]|| (p_dn - dn) == space1[2]|| (p_dn - dn) == space1[3]))
					 )
					 ||
					 (
						(m1 == 0) && (m0 == 1 && ((p_dn0 - dn) == space0[0] || (p_dn0 - dn) == space0[1]|| (p_dn0 - dn) == space0[2]|| (p_dn0 - dn) == space0[3]))
					 )
				   )
			{
				if((m0 == 1 && (ia != space0[0] && ia != space0[1]&& ia != space0[2]&& ia != space0[3])) )
				{
				   sq = sq2;
				   ps = ps2;
				   alp = alp2;
				...
				if((m1 == 1 && (ib != space1[0] && ib != space1[1] && ib != space1[2] && ib != space1[3])) )
				{
				   sq = sq2;
				   ps = ps2;
				   alp = alp2;
			...
			/*printf("%u=%u-%u  %u\n",p_dn0-dn,p_dn0,dn,p_dn_max);
			printf("%u=%u-%u  %u\n\n",p_dn-dn,p_dn,dn,p_dn_max);*/
            p_rrv += 5;
            p_dn += 5;
            p_r4 += 5;
         } while ( p_dn < p_dn_max );
	  ...
	  if(mark[i4%5]!=-1)
	  {
		  r_emrate = ten_em[i4%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m0=1;
			  searchSpace(mark[i4%5],space0,r_msg,enta);
		  }
	  }
      if(mark[i5%5]!=-1)
	  {
		  r_emrate = ten_em[i5%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m1=1;
			  searchSpace(mark[i5%5],space1,r_msg,enta);
		  }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r5 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
			...
			if(m0 == 0 && m1 == 0)
			{
				if ( ( alp * sq2 ) > ( sq * alp2 ) ) {
				   ...
				   ib = ( Word16 )( p_dn - dn );
					 ...
					 (
						(m0 == 1 && ((p_dn0 - dn) == space0[0] || (p_dn0 - dn) == space0[1] || (p_dn0 - dn) == space0[2] || (p_dn0 - dn) == space0[2])) 
						&& 
						(m1 == 1 && ((p_dn  - dn) == space1[0] || (p_dn  - dn) == space1[1] || (p_dn  - dn) == space1[2] || (p_dn  - dn) == space1[3]))
					 )
					 ||
	  ...
	  if(mark[i6%5]!=-1)
	  {
		  r_emrate = ten_em[i6%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m0=1;
			  searchSpace(mark[i6%5],space0,r_msg,enta);
		  }
	  }
      if(mark[i7%5]!=-1)
	  {
		  r_emrate = ten_em[i7%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m1=1;
			  searchSpace(mark[i7%5],space1,r_msg,enta);
		  }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r7 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
	  ...
	  if(mark[i8%5]!=-1)
	  {
		  r_emrate = ten_em[i8%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m0=1;
			  searchSpace(mark[i8%5],space0,r_msg,enta);
		  }
	  }
      if(mark[i9%5]!=-1)
	  {
		  r_emrate = ten_em[i9%5];  
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,enta*2));
			  m1=1;
			  searchSpace(mark[i9%5],space1,r_msg,enta);
		  }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r9 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
      ...
      if ( ( alpk * sq ) > ( psk * alp ) ) {
		...
		/* for(t= 0; t<10;t++)
		 {
			 printf("i%d: %d in track %d \n",t,codvec[t],codvec[t]%5);
		 }
		 printf("\n");
		 system("pause");*/
      }
...
static void build_code_10i40_35bits( Word32 codvec[], Float32 dn_sign[], Float32
      cod[], Float32 h[], Float32 y[], Word16 indx[], int enta)
{
 ...
 //  for(t= 0; t<5;t++)
	//{
	//	fprintf(FCIrec,"%d %d ",(5*(indx[t] >= 8 ? indx[t]-8 : indx[t])+t), (5*(indx[t+5] >= 8 ? indx[t+5]-8 : indx[t+5])+t));
	//	//printf("%d,%d,",(5*(indx[t] >= 8 ? indx[t]-8 : indx[t])+t), (5*(indx[t+5] >= 8 ? indx[t+5]-8 : indx[t+5])+t));	
	//	//printf("-----%d\n",getMsg((5*(indx[t] >= 8 ? indx[t]-8 : indx[t])+t),(5*(indx[t+5] >= 8 ? indx[t+5]-8 : indx[t+5])+t),enta));
	//	//fprintf(FCIrec,"%d,%d,",(5*(indx[t] >= 8 ? indx[t]-8 : indx[t])+t)*(indx[t] >= 8 ? -1 : 1), (5*(indx[t+5] >= 8 ? indx[t+5]-8 : indx[t+5])+t)*(indx[t+5] >= 8 ? -1 : 1));
	//	//fprintf(FCIrec,"%d,%d,",indx[t],indx[t+5]);
	//	
	//}
 //  fprintf(FCIrec,"\n");
 //  //system("pause");
...
static void code_10i40_35bits( Float32 x[], Float32 cn[], Float32 h[],
    ...
    Float32 y[], Word16 anap[] ,int em_rate,int enta)
 {
    Float32 rr[L_CODE][L_CODE];
    Float32 dn[L_CODE], sign[L_CODE];
    ...
    */
    ...
    cor_h( h, sign, rr );
    search_10i40(dn, rr, ipos, pos_max, codvec ,em_rate,enta);
    build_code_10i40_35bits( codvec, sign, code, h, y, anap,enta );
...
static void cbsearch( enum Mode mode, Word16 subnr, Float32 x[],
                     ...
                     Float32 *res2, Word16 **anap, int em_rate, int enta)
{
   switch (mode){
   case MR475:
   case MR515:
   ...
   case MR102:
      ...
      *anap += 7;
      break;
   default:
      code_10i40_35bits( x, res2, h, T0, gain_pit, code, y, *anap, em_rate,enta);
      *anap += 10;
   }
}
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[], int em_rate,int enta)
{
   /* LPC coefficients */
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode, int em_rate ,int enta)
{
   ...
   cod_amr( state->cod_amr_state, mode, speech, prm, used_mode, syn, em_rate,enta);
