...
int main(int argc, char *argv[] ) {
	...
	for(em_rate=100;em_rate<=100;em_rate=em_rate+5)
	{
		for(filenu=0;filenu<1;filenu++)//混合样本有3367个
		{			
			sprintf(inFileName,"F:\\sample\\original_pcmfiles\\all\\sample%04d.pcm",filenu);
			sprintf(outFileName,"D:\\data\\AMR_NB_FCB_GEISER\\3367\\10s\\%d\\geiser\\sample%04d.dat",em_rate,filenu);
 ...
 *    0
 */
int AMRcoder (char * inFileName, char * outFileName, int em_rate){
  ...
  // }
   ...
   while (fread( speech, sizeof (Word16), 160, file_speech ) > 0)
   {
      ...
      /* call encoder */
      byte_counter = Encoder_Interface_Encode(enstate, req_mode, speech, serial_data, 0,em_rate);
