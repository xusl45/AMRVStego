...
FILE *secret;
int js=1,jsj=0,lag_last=0,sec=0;
int lag_now=0;
int secretmassege=0;
      ...
      , Word16 *resu3, Word32 *ana_index )
{
   ...
   //开始隐写操作
   secretmassege=rand()%2;

   if(js==1)*T0_prev_subframe=lag;//第一个基音延迟,因为前面没有可以减的
   if (abs(*T0_prev_subframe-lag)>5)//嵌入的条件为大于六
   {
	   ste = 1; 
	   tempLength++;jsj++;	   
   }
   lag_now=lag;//为下面的控制做准备
   if(ste==1)
   {	  
		if((lag%2)!=(secretmassege))//如果与秘密潜入信息不符合就重新搜索
		{   
			max = corr[T0_min];
			lag = T0_min;
			for ( i = T0_min + 1; i <= T0_max; i++ ) 
			{
				if ( (corr[i] >= max) && (i%2==secretmassege)
					&&(abs(i-*T0_prev_subframe)>5))//嵌入隐秘信息条件,符合奇偶，大于等于现在
				{max = corr[i];lag = i;}				
			 }
			if(lag==T0_min)
			{
				if(abs(*T0_prev_subframe-lag_now)==6)
				{
					if(lag_now>*T0_prev_subframe)
						{if(lag_now==143){lag=lag_now;}else lag=lag_now+1;}
					else					
						{if(lag_now==18){lag=lag_now;}else lag=lag_now-1;}
				}
				if(abs(*T0_prev_subframe-lag_now)>6)
				{
					if(lag_now>*T0_prev_subframe)lag=lag_now-1;
					else lag=lag_now+1;
				}
			}
		}		
...
void searchSpace(Word32 pulse,Word32 *space,Word8 msg)
{
	Word32 temp;
	//msg=1;
	//printf("msg: %d\n",msg);
...
static void search_10i40( Float32 dn[], Float32 rr[][L_CODE], Word32 ipos[],
      Word32 pos_max[], Word32 codvec[], int em_rate )
{
   Float32 rrv[L_CODE];
   Float32 psk, ps, ps0, ps1, ps2, sq, sq2, alpk, alp, alp0, alp1, alp2;
   Float32 *p_r, *p_r0, *p_r1, *p_r2, *p_r3, *p_r4, *p_r5, *p_r6, *p_r7, *p_r8,
   ...
   Word32 i0, i1, i2, i3, i4, i5, i6, i7, i8, i9, j, k, ia, ib, i, pos;
	...
	Word32 space0[2], space1[2];
   int t,mark[5],m0,m1,r_emrate,r_msg,ten_em[5];
   
   p_dn_max = &dn[39];

   /* fix i0 on maximum of correlation position */
   i0 = pos_max[ipos[0]];/*iposc传进来的位置*/
   ia = ib = 0;
   ps = 0;
   ...
   //每一轨道生成随机数
   for ( i = 0; i < 5; i++ ) {
	   ten_em[i] = (int)(random(0,100));  /*确定哪一个轨道隐写*/
	   //ten_em[i] = 100;
   }
   //ten_em[0]=5;
   ...
   for ( i = 1; i < 5; i++ ) {
	  ...
	  //set i1 mark
	  mark[i1%5]=i1;/*这个i1取值是1，6，11，16，21*/
	  ...
	  if(mark[i2%5]!=-1)
	  {
		  ...
		  if(r_emrate < em_rate)
	      {   
			  r_msg    = (unsigned int)(random(0,4));
		      m0=1;
		      searchSpace(mark[i2%5],space0,r_msg);/*这个地方是进行信息隐藏的地方*/
		  }
	  }
      if(mark[i3%5]!=-1)
	  {
		   ...
		   if(r_emrate < em_rate)
	       {
			  r_msg    = (unsigned int)(random(0,4));
			  m1=1;
              searchSpace(mark[i3%5],space1,r_msg);/*这个地方是进行信息隐藏的地方*/
		   }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r3 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
			...
			if(m0 == 0 && m1 == 0)
			{
				if ( ( alp * sq2 ) > ( sq * alp2 ) ) {
				   ...
				   ib = ( Word16 )( p_dn - dn );
					 ...
					 (
						(m0 == 1 && ((p_dn0 - dn) == space0[0] || (p_dn0 - dn) == space0[1])) 
						&& 
						(m1 == 1 && ((p_dn  - dn) == space1[0] || (p_dn  - dn) == space1[1]))
					 )
					 ||
					 (
						(m0 == 0) && (m1 == 1 && ((p_dn  - dn) == space1[0] || (p_dn  - dn) == space1[1]))
					 )
					 ||
					 (
						(m1 == 0) && (m0 == 1 && ((p_dn0 - dn) == space0[0] || (p_dn0 - dn) == space0[1]))
					 )
				   )
			{
				if((m0 == 1 && (ia != space0[0] && ia != space0[1])) )
				{
				   sq = sq2;
				   ps = ps2;
				   alp = alp2;
				...
				if((m1 == 1 && (ib != space1[0] && ib != space1[1])) )
				{
				   sq = sq2;
				   ps = ps2;
				   alp = alp2;
	  ...
	  if(mark[i4%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m0=1;
              searchSpace(mark[i4%5],space0,r_msg);/*这个地方就是信息隐藏的位置*/
		  }
	  }
      if(mark[i5%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m1=1;
			  searchSpace(mark[i5%5],space1,r_msg);
		  }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r5 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
	  ...
	  if(mark[i6%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m0=1;
			  searchSpace(mark[i6%5],space0,r_msg);
		  }
	  }
      if(mark[i7%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m1=1;
			  searchSpace(mark[i7%5],space1,r_msg);
		  }
	  }
      ...
      *( p_rrv + 35 ) = *( p_r7 + 1435 ) + 2.0F * ( *( p_r0 + 35 ) + *( p_r1 +
	  ...
	  if(mark[i8%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m0=1;
			  searchSpace(mark[i8%5],space0,r_msg);
		  }
	  }
      if(mark[i9%5]!=-1)
	  {
	      ...
	      {   
			  r_msg    = (unsigned int)(random(0,4));
			  m1=1;
			  searchSpace(mark[i9%5],space1,r_msg);
		  }
	  }
...
Word32 getMsg(int pulse0,int pulse1)
{
	Word32 msg;
      ...
      cod[], Float32 h[], Float32 y[], Word16 indx[])
{
 ...
 //  for(t= 0; t<5;t++)
	...
	//	//printf("%d,%d,",(5*(indx[t] >= 8 ? indx[t]-8 : indx[t])+t)*(indx[t] >= 8 ? -1 : 1), (5*(indx[t+5] >= 8 ? indx[t+5]-8 : indx[t+5])+t)*(indx[t+5] >= 8 ? -1 : 1));	
	//}
 //  fprintf(FCIrec,"\n");
 //  //system("pause");
...
static void code_10i40_35bits( Float32 x[], Float32 cn[], Float32 h[],
    ...
    Float32 y[], Word16 anap[] ,int em_rate)
 {
    Float32 rr[L_CODE][L_CODE];
    Float32 dn[L_CODE], sign[L_CODE];
    ...
    */
    ...
    cor_h( h, sign, rr );
    search_10i40(dn, rr, ipos, pos_max, codvec ,em_rate);
...
static void cbsearch( enum Mode mode, Word16 subnr, Float32 x[],
                     ...
                     Float32 *res2, Word16 **anap, int em_rate)/*传进来的参数，这些输入的subnr,x[],h[],T0, */
{
   switch (mode){
   case MR475:
   case MR515:
   ...
   case MR102:
      ...
      *anap += 7;
      break;
   default:
      code_10i40_35bits( x, res2, h, T0, gain_pit, code, y, *anap, em_rate);/*12.2速率部分的隐写代码*/
      *anap += 10;
   }
}
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[], int em_rate)
{
   /* LPC coefficients */
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode, int em_rate)
{
   ...
   cod_amr( state->cod_amr_state, mode, speech, prm, used_mode, syn, em_rate);
