...
/*
	@author: Qiu yiqin
	@date: 2021.08.12
	@func: Implementation of DN-QIM. Based on: A Novel AMR-WB Speech Steganography Based on Diameter-Neighbor Codebook Partition. 2018
*/
...
float getDistance(Cluster left, Cluster right, int dim) {
	float dist = 0;
	for (int i = 0; i < dim; i++) {
		dist += pow((left.centroid[i] - right.centroid[i]), 2);
	}
	return sqrtf(dist);
}

int getNeighbor(Cluster *ClusterSet, int dico_size, int pos, int dim) {
	int min = 2147483647;
	int neighbor_pos = pos;
	for (int i = 0; i < dico_size; i++) {
		if (i != pos && ClusterSet[i].num != 0) {
			int dist = getDistance(ClusterSet[pos], ClusterSet[i], dim);
			if (dist < min) {
				neighbor_pos = i;
				min = dist;
			}
...
Cluster *init_ClusterSet(Cluster *ClusterSet, int dico_size, Float32 *dico_lsf, int dim) {
	ClusterSet = (Cluster*)malloc(sizeof(Cluster) * dico_size);
	for (int i = 0; i < dico_size; i++) {
		for (int j = 0; j < 4; j++) {
			if (j < dim) {
				int index = i * dim;
				ClusterSet[i].centroid[j] = dico_lsf[index + j];
			}
			else {
				ClusterSet[i].centroid[j] = 0;
			}
		}
		ClusterSet[i].IndexSet[0] = i;
		ClusterSet[i].num = 1;
		for (int j = 1; j < 256; j++) {
			ClusterSet[i].IndexSet[j] = -1;
		}
	}
	return ClusterSet;
}

Cluster *init_TempSet(Cluster *ClusterSet, int dico_size) {
	for (int i = 0; i < dico_size; i++) {
		for (int j = 0; j < 4; j++) {
			ClusterSet[i].centroid[j] = 0;
		}
...
bool isEmpty(Cluster *ClusterSet, int dico_size) {
	for (int i = 0; i < dico_size; i++) {
		if (ClusterSet[i].num != 0) {
			return false;
		}
	}
	return true;
...
Cluster *DN_Division(Cluster *ClusterSet, int N, Float32 *dico_lsf, int dico_size, int dim) {
	Cluster *HelperSet = (Cluster*)malloc(sizeof(Cluster) * dico_size);
	HelperSet = init_TempSet(HelperSet, dico_size);
	int helperset_size = 0;
	while (N > 0) {
		if (isEmpty(ClusterSet, dico_size) == true) {
			ClusterSet = init_TempSet(ClusterSet, dico_size);
			for (int i = 0; i < helperset_size; i++) {
				ClusterSet[i].num = HelperSet[i].num;
				for (int k = 0; k < HelperSet[i].num; k++) {
					ClusterSet[i].IndexSet[k] = HelperSet[i].IndexSet[k];
				}
				for (int k = 0; k < dim; k++) {
					ClusterSet[i].centroid[k] = HelperSet[i].centroid[k];
				}
			}
			HelperSet = init_TempSet(HelperSet, dico_size);
			helperset_size = 0;
		...
		//get clusters of the diameter cluster pair
		for (int i = 0; i < dico_size - 1; i++) {
			if (ClusterSet[i].num != 0) {
				for (int j = i + 1; j < dico_size; j++) {
					if (ClusterSet[j].num != 0) {
						int dist = getDistance(ClusterSet[i], ClusterSet[j], dim);
						if (dist > diameter) {
							left_pos = i;
							right_pos = j;
							diameter = dist;
		...
		//get the neighbor of a cluster and combine it with this cluster
		int left_neighbor = getNeighbor(ClusterSet, dico_size, left_pos, dim);
		int Temp1_size = ClusterSet[left_pos].num + ClusterSet[left_neighbor].num;
		for (int i = 0; i < Temp1_size; i++) {
			if (i < ClusterSet[left_pos].num) {
				HelperSet[helperset_size].IndexSet[i] = ClusterSet[left_pos].IndexSet[i];
			}
			else {
				HelperSet[helperset_size].IndexSet[i] = ClusterSet[left_neighbor].IndexSet[i - ClusterSet[left_pos].num];
			}
			int pos = HelperSet[helperset_size].IndexSet[i] * dim;
			for (int j = 0; j < dim; j++) {
				HelperSet[helperset_size].centroid[j] += dico_lsf[pos + j];
			}
		}
		for (int i = 0; i < ClusterSet[left_pos].num; i++) {
			ClusterSet[left_pos].IndexSet[i] = -1;
		}
		for (int i = 0; i < ClusterSet[left_neighbor].num; i++) {
			ClusterSet[left_neighbor].IndexSet[i] = -1;
		}
		ClusterSet[left_pos].num = 0;
		ClusterSet[left_neighbor].num = 0;

		int right_neighbor = getNeighbor(ClusterSet, dico_size, right_pos, dim);
		int Temp2_size = ClusterSet[right_pos].num + ClusterSet[right_neighbor].num;
		for (int i = 0; i < Temp2_size; i++) {
			if (i < ClusterSet[right_pos].num) {
				HelperSet[helperset_size + 1].IndexSet[i] = ClusterSet[right_pos].IndexSet[i];
			}
			else {
				HelperSet[helperset_size + 1].IndexSet[i] = ClusterSet[right_neighbor].IndexSet[i - ClusterSet[right_pos].num];
			}
			int pos = HelperSet[helperset_size + 1].IndexSet[i] * dim;
			for (int j = 0; j < dim; j++) {
				HelperSet[helperset_size + 1].centroid[j] += dico_lsf[pos + j];
			}
		}
		for (int j = 0; j < dim; j++) {
			HelperSet[helperset_size].centroid[j] /= Temp1_size;
			HelperSet[helperset_size + 1].centroid[j] /= Temp2_size;
		}
		HelperSet[helperset_size].num = Temp1_size;
		HelperSet[helperset_size + 1].num = Temp2_size;
		helperset_size += 2;
		for (int i = 0; i < dim; i++) {
			ClusterSet[left_pos].centroid[i] = 0;
			ClusterSet[left_neighbor].centroid[i] = 0;
			ClusterSet[right_pos].centroid[i] = 0;
			ClusterSet[right_neighbor].centroid[i] = 0;
		}
		for (int i = 0; i < ClusterSet[right_pos].num; i++) {
			ClusterSet[right_pos].IndexSet[i] = -1;
		}
		for (int i = 0; i < ClusterSet[right_neighbor].num; i++) {
			ClusterSet[right_neighbor].IndexSet[i] = -1;
		}
		ClusterSet[right_pos].num = 0;
		ClusterSet[right_neighbor].num = 0;
		N--;
	}
	for (int i = 0; i < helperset_size; i++) {
		for (int j = 0; j < dico_size; j++) {
			if (ClusterSet[j].num == 0) {
				ClusterSet[j].num = HelperSet[i].num;
				for (int k = 0; k < HelperSet[i].num; k++) {
					ClusterSet[j].IndexSet[k] = HelperSet[i].IndexSet[k];
				}
				for (int k = 0; k < dim; k++) {
					ClusterSet[j].centroid[k] = HelperSet[i].centroid[k];
				}
				break;
			}
...
int getClusterPos(Cluster *ClusterSet, int index, int dico_size) {
	int pos = 0;
	for (int i = 0; i < dico_size; i++) {
		if (ClusterSet[i].num != 0) {
			for (int j = 0; j < ClusterSet[i].num; j++) {
				if (ClusterSet[i].IndexSet[j] == index) {
					pos = i;
					return pos;
				}
			}
...
Word16 E_LPC_isf_sub_vq_s(Float32 *x, const Float32 *E_ROM_dico, Word32 dim,
	Word32 E_ROM_dico_size, Float32 *distance, Cluster *ClusterSet, int emrate, int *changeframe)
{
	Float32 dist_min, dist, temp;
	const Float32 *p_E_ROM_dico;
	Word32 i, j, index = 0;
	...
	*distance = dist_min;
	...
	if (rand() % 10 < emrate / 10) {
		*changeframe = *changeframe + 1;
		int pos = getClusterPos(ClusterSet, index, E_ROM_dico_size);
		int bit = rand() % 2;
			
		//index = ClusterSet[pos].IndexSet[rand() % (ClusterSet[pos].num)];
		index = ClusterSet[pos].IndexSet[bit];
		//printf("%d ", bit);
...
void E_LPC_isf_2s5s_quantise(Float32 *isf1, Word16 *isf_q, Word16 *past_isfq,
                             Word32 *indice, Word32 nb_surv, int em_rate, int *changeframe)
{
   Float32 isf[ORDER], isf_stage2[ORDER];
   Float32 temp, min_err, distance;
   Word32 surv1[N_SURV_MAX];     /* indices of survivors from 1st stage */
   ...
   if (isPartition == false) {
	   ClusterSet1 = init_ClusterSet(ClusterSet1, SIZE_BK21, E_ROM_dico21_isf, 3);
	   ClusterSet2 = init_ClusterSet(ClusterSet2, SIZE_BK22, E_ROM_dico22_isf, 3);
	   ClusterSet3 = init_ClusterSet(ClusterSet3, SIZE_BK23, E_ROM_dico23_isf, 3);
	   ClusterSet4 = init_ClusterSet(ClusterSet4, SIZE_BK24, E_ROM_dico24_isf, 3);
	   ClusterSet5 = init_ClusterSet(ClusterSet5, SIZE_BK25, E_ROM_dico25_isf, 4);
	   if (ClusterSet1 == NULL || ClusterSet2 == NULL || ClusterSet3 == NULL || ClusterSet4 == NULL || ClusterSet5 == NULL)
	   {
		   printf("malloc fail!\n");
		   exit(0);
	   }
	   ClusterSet1 = DN_Division(ClusterSet1, 16, E_ROM_dico21_isf, SIZE_BK21, 3);
	   ClusterSet2 = DN_Division(ClusterSet2, 32, E_ROM_dico22_isf, SIZE_BK22, 3);
	   ClusterSet3 = DN_Division(ClusterSet3, 32, E_ROM_dico23_isf, SIZE_BK23, 3);
	   ClusterSet4 = DN_Division(ClusterSet4, 8, E_ROM_dico24_isf, SIZE_BK24, 3);
	   ClusterSet5 = DN_Division(ClusterSet5, 8, E_ROM_dico25_isf, SIZE_BK25, 4);
	   isPartition = true;
   }
   ...
   for (k = 0; k < nb_surv; k++)
   {
      ...
      tmp_ind[0] = E_LPC_isf_sub_vq_s(&isf_stage2[0], E_ROM_dico21_isf, 3,
         SIZE_BK21, &min_err, ClusterSet1, em_rate, changeframe);
      temp = min_err;
      tmp_ind[1] = E_LPC_isf_sub_vq_s(&isf_stage2[3], E_ROM_dico22_isf, 3,
         SIZE_BK22, &min_err, ClusterSet2, em_rate, changeframe);
      temp += min_err;
      tmp_ind[2] = E_LPC_isf_sub_vq_s(&isf_stage2[6], E_ROM_dico23_isf, 3,
         SIZE_BK23, &min_err, ClusterSet3, em_rate, changeframe);
      temp += min_err;
   ...
   for (k=0; k<nb_surv; k++)
   {
      ...
      tmp_ind[0] = E_LPC_isf_sub_vq_s(&isf_stage2[0], E_ROM_dico24_isf, 3,
         SIZE_BK24, &min_err, ClusterSet4, em_rate, changeframe);
      temp = min_err;
      tmp_ind[1] = E_LPC_isf_sub_vq_s(&isf_stage2[3], E_ROM_dico25_isf, 4,
         SIZE_BK25, &min_err, ClusterSet5, em_rate, changeframe);
      temp += min_err;
      if (temp < distance)
      {
         distance = temp;
