...
//////////////一些需要的函数实现/////////////////
//////////////////////////////////////////////////
#define maxDist 4*pow((float)1000,2)
typedef struct      //定义点，保存索引值，码字，标签
{
	int index;

	int label;

	int flag;  //判断label是否修改
...
int LabelSetting(int label)
{
	return (label == 0?1:0);
}

float EuDistance(Vertex v1, Vertex v2)  //欧式距离
{
	int i;
	float sum = 0;
	float dist;
	for (i = 0; i < 4; i++)
	{
		sum += pow((v1.codebook[i] - v2.codebook[i]),2);
	}
	dist = sqrt(sum);
	return dist; 
}

Vertex getCNV(Vertex cnvGraph[], short dico_size, Vertex v) //返回点v的最近邻点
{
	float dist, mindist;
	int dex,i;
	mindist = maxDist;
	...
	{
		if (v.index != cnvGraph[i].index) //不是同一个点
		{
			dist = EuDistance(cnvGraph[i], v);
			if (dist < mindist)
			{
				mindist = dist;
				dex = i;
...
void CNVSub_Graphy(const Float32 *dico, Word16 dico_size, Vertex cnvGraph[])     //CNV子图构建算法
{
	//初始化：每个组原点的label为0，其最近点label为1
	//用-1初始化各个标签label
	...
	for (i = 0; i < dico_size; i++)
	{
		v = getCNV(cnvGraph, dico_size, cnvGraph[i]);
		cnvGraph[i].cnvIndex = v.index;
		if (cnvGraph[cnvGraph[i].cnvIndex].flag == -1)
		{		
			cnvGraph[cnvGraph[i].cnvIndex].label = LabelSetting(cnvGraph[i].label);
			cnvGraph[cnvGraph[i].cnvIndex].flag = 1;
			cnvGraph[i].flag = 1;
		}
		else
		{
			cnvGraph[i].label = LabelSetting(cnvGraph[cnvGraph[i].cnvIndex].label);
			cnvGraph[cnvGraph[i].cnvIndex].flag = 1;
			cnvGraph[i].flag = 1;
		}
	}
...
Word16 CNV(Vertex cnvGraph[], Word16 index, int label)       //CNV隐写算法
{
	if (label != cnvGraph[index].label)  //当前index对应的label和秘密信息（label）不相等
	{
		return (cnvGraph[index].cnvIndex);  //返回最近邻点的index
	}
	return (Word16)index;  //否则返回当前index
}
//////////////////////////////////////////////////////////////////////////////////////////
//重要的变量
Vertex *cnvGraph1, *cnvGraph2, *cnvGraph3, *cnvGraph4, *cnvGraph5;   //保存五个码字表的划分结构
int notExist = 1;
//////////////////////////////////////////////////////////////////////////////////////////
...
static Word16 Vq_subvec( Float32 *lsf_r1, Float32 *lsf_r2, const Float32 *dico,
      Float32 *wf1, Float32 *wf2, Word16 dico_size, Vertex * cnvGraphy, int emrate )
{
   ...
   //进行隐写
   //嵌入率emrate，秘密信息msgbit = rand()%2，索引index，是否匹配rand()100 < emrate && msgbit，否则，利用CNV算法求解最近的索引值 
   ////
   if (rand()%100 < emrate)
   {
	   index =  CNV(cnvGraphy, index, rand()%2);
   }
...
static void Q_plsf_5( Float32 *past_rq, Float32 *lsp1, Float32 *lsp2, Float32 *
      lsp1_q, Float32 *lsp2_q, Word16 *indice, int emrate )
{
   ...
   /* Compute predicted LSF and prediction error */
   for ( i = 0; i < M; i++ ) {
      /* MR122 LSP prediction factor = 0.65 */                  ///减去均值mean_lsf_5，，再用一阶滑动平均（MA）预测法求残差矢量
      lsf_p[i] = mean_lsf_5[i] + past_rq[i] * 0.65F;
      lsf_r1[i] = lsf1[i] - lsf_p[i];
      lsf_r2[i] = lsf2[i] - lsf_p[i];
   }

   //获取码字分布，执行一次，提升代码执行效率
   if (notExist)
   {
	   cnvGraph1 = (Vertex*)malloc(sizeof(Vertex)*DICO1_SIZE_5);
	   cnvGraph2 = (Vertex*)malloc(sizeof(Vertex)*DICO2_SIZE_5);
	   cnvGraph3 = (Vertex*)malloc(sizeof(Vertex)*DICO3_SIZE_5);
	   cnvGraph4 = (Vertex*)malloc(sizeof(Vertex)*DICO4_SIZE_5);
	   cnvGraph5 = (Vertex*)malloc(sizeof(Vertex)*DICO5_SIZE_5);
	   if(cnvGraph1 == NULL || cnvGraph2 == NULL || cnvGraph3 == NULL || cnvGraph4 == NULL || cnvGraph5 == NULL)
	   {
		   printf("No enough memory !\n");
		   exit(0);
	   }
	   CNVSub_Graphy(dico1_lsf_5, DICO1_SIZE_5, cnvGraph1);
	   CNVSub_Graphy(dico2_lsf_5, DICO2_SIZE_5, cnvGraph2);
	   CNVSub_Graphy(dico3_lsf_5, DICO3_SIZE_5, cnvGraph3);
	   CNVSub_Graphy(dico4_lsf_5, DICO4_SIZE_5, cnvGraph4);
	   CNVSub_Graphy(dico5_lsf_5, DICO5_SIZE_5, cnvGraph5);
	   notExist = 0;
   }


   /* Split-MQ of prediction error */                          ///分裂矢量量化（SMQ）
   indice[0] = Vq_subvec( &lsf_r1[0], &lsf_r2[0], dico1_lsf_5, &wf1[0], &wf2[0],
         DICO1_SIZE_5, cnvGraph1, emrate );
   indice[1] = Vq_subvec( &lsf_r1[2], &lsf_r2[2], dico2_lsf_5, &wf1[2], &wf2[2],
         DICO2_SIZE_5, cnvGraph2, emrate );
   indice[2] = Vq_subvec_s( &lsf_r1[4], &lsf_r2[4], dico3_lsf_5, &wf1[4], &wf2[4],
	     DICO3_SIZE_5, cnvGraph3, emrate );
   indice[3] = Vq_subvec( &lsf_r1[6], &lsf_r2[6], dico4_lsf_5, &wf1[6], &wf2[6],
         DICO4_SIZE_5, cnvGraph4, emrate );
   indice[4] = Vq_subvec( &lsf_r1[8], &lsf_r2[8], dico5_lsf_5, &wf1[8], &wf2[8],
         DICO5_SIZE_5, cnvGraph5, emrate );
...
static void lsp( enum Mode req_mode, enum Mode used_mode, Float32 *lsp_old,
      ...
      lsp_new[], Word16 **anap, int emrate )
{
   ...
   if ( req_mode == MR122 ) {
      ...
      if ( used_mode != MRDTX ) {
         ...
         Q_plsf_5( past_rq, lsp_mid, lsp_new, lsp_mid_q, lsp_new_q, *anap, emrate );
...
static void cod_amr( cod_amrState *st, enum Mode mode, Float32 new_speech[],
      Word16 ana[], enum Mode *used_mode, Float32 synth[], int emrate )
{
   /* LPC coefficients */
   ...
   lsp( mode, *used_mode, st->lspSt->lsp_old, st->lspSt->lsp_old_q, st->lspSt->
         qSt->past_rq, A_t, Aq_t, lsp_new, &ana, emrate );
...
void Speech_Encode_Frame( void *st, enum Mode mode, Word16 *new_speech, Word16 *
      prm, enum Mode *used_mode, int emrate )
{
   ...
   cod_amr( state->cod_amr_state, mode, speech, prm, used_mode, syn, emrate );
