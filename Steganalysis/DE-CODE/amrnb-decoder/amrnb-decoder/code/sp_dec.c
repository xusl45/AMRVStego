...
FILE *AMRNB_file,*ACB_file,*ACB_gain_file,*FCB_file,*FCB_gain_file;
FILE *lag_d;
int js=0;
int steBit;
/*
...
static void D_plsf_5( D_plsfState *st, Word16 bfi, Word16 *indice, Word32 *lsp1_q
      , Word32 *lsp2_q ,int LSP_parameter[])
{
   ...
   for(i=0;i<5;i++)LSP_parameter[i]=indice[i];
   /*printf("%s\t%d\t%d\t%d\t%d\t%d\n","LPC",indice[0],
 ...
 *    void
 */
static void decode_10i40_35bits( Word16 index[], Word32 cod[], int FCB_pulse_subf[])
{
   ...
   for ( j = 0; j < 5; j++ ) {
      ...
      /* position of pulse "j" */
      pos1 = ( i + j );
	  //printf("i%d: %d ",j,index[j]);
      i = ( tmp >> 3 ) & 1;
      if ( i == 0 ) {sign = 4096;   /* +1.0 */}
      else {sign = -4096;   /* -1.0 */}
	  //printf("i%d: %c%d ",j,(sign > 0 ? '+' : '-'),pos1);
      /* compute index i */
      i = index[j + 5] & 7;
      i = dgray[i];
      i = i * 5;
      ...
      pos2 = ( i + j );	  
	  /*printf("i%d: %d ",j+5,index[j+5]);*/
      cod[pos1] = sign;
      if ( pos2 < pos1 ) {
         sign = -( sign );
      }

	  //printf("i%d: %c%d ",j+5,(sign > 0 ? '+' : '-'),pos2);

      cod[pos2] = cod[pos2] + sign;
	  //printf("msg:%d \n",(gray[pos1/5]^gray[pos2/5])%4);
	  /*printf("%d,%d,",pos1,pos2);*/
	  FCB_pulse_subf[2*j]=pos1;
	  FCB_pulse_subf[2*j+1]=pos2;
   }/*printf("\t");*/
 ...
 *    exp_gcode0              O: predicted gain factor (exponent指数)
 *    frac_gcode0             O: predicted gain factor (fraction小数)
...
static void Decoder_amr( Decoder_amrState *st, enum Mode mode, Word16 parm[],
	  ...
	  int LSP_parameter[],int ACB_integer[],int ACB_fraction[],int ACB_Gain[],
	  int ACB_Gain_index[],int FCB_pulse[][10],int FCB_Gain_index[],int FCB_Gain[])
{
	//要输出的参数
	//int LSP_parameter[5]={0};
	//int ACB_integer[4]={0},ACB_fraction[4]={0};
	/*int ACB_inte_subf=0,ACB_frac_subf=0;
	int ACB_Gain[4]={0},ACB_Gain_index[4]={0};
	int FCB_pulse[4][10]={0};*/
	int FCB_pulse_subf[10]={0};
	/*int FCB_Gain_index[4]={0};
	int FCB_Gain[4]={0};*/
	int countnumber=0;
   ...
   if ( mode != MR122 ) {
      ...
      Int_lpc_1to3( st->lsp_old, lsp_new, A_t );
	   ...
	   //此处输出LPC参数
      D_plsf_5( st->lsfState, bfi, parm, lsp_mid, lsp_new ,LSP_parameter);
	  //输出LSP参数2018.11.29
	 /* printf("%s\t%d\t%d\t%d\t%d\t%d\n","LPC",LSP_parameter[0],
	   LSP_parameter[1],LSP_parameter[2],LSP_parameter[3],LSP_parameter[4]);*/
      /* Advance synthesis parameters pointer */
      parm += 5;
   ...
   for ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR ) {
      ...
      if ( mode != MR122 ) {
         ...
         Dec_lag6( index, PIT_MIN_MR122, PIT_MAX, pit_flag, &T0, &T0_frac);
		  //输出lag
		 ACB_integer[i_subfr/L_SUBFR]=T0;
		 ACB_fraction[i_subfr/L_SUBFR]=T0_frac;
		 //输出ACB整数和小数基音延迟2018.11.29
		 /*printf("%s\t%d\t%d\t","ACB",
			 ACB_integer[i_subfr/L_SUBFR],ACB_fraction[i_subfr/L_SUBFR]);*/
      ...
      /* MR122 */
         ...
         if ( bfi != 0 ) {
			...
			ACB_Gain[i_subfr/L_SUBFR]=gain_pit;
			ACB_Gain_index[i_subfr/L_SUBFR]=index;
			//输出ACB增益和增益系数2018.11.29
			/*printf("%s\t%d\t%s\t%d\t",
			"ACBGain",ACB_Gain[i_subfr/L_SUBFR],
			"ACBindex",ACB_Gain_index[i_subfr/L_SUBFR]);*/
         }
         ec_gain_pitch_update( st->ec_gain_p_st, bfi, st->prev_bf, &gain_pit );

		 //此处输出FCB码本参数
         decode_10i40_35bits( parm, code ,FCB_pulse_subf);
		 //输出FCB脉轨参数2018.11.29
		 for (countnumber=0;countnumber<10;countnumber++)
		 {
			/*printf("%d ",FCB_pulse_subf[countnumber]);*/
			FCB_pulse[i_subfr/L_SUBFR][countnumber]=FCB_pulse_subf[countnumber];
		 }/*printf("\t");*/
      ...
      if ( mode == MR475 ) {
         ...
         if ( mode == MR795 ) {
            ...
            if ( bfi == 0 ) {
				//此处输出固定码本增益
               d_gain_code( st->pred_state, mode, index, code, &gain_code);
			   FCB_Gain[i_subfr/L_SUBFR]=gain_code;
			   FCB_Gain_index[i_subfr/L_SUBFR]=index;
			   //输出FCB增益系数和增益2018.11.29
			  /* printf("%s\t%d\t%s\t%d\n","FCBGain",FCB_Gain[i_subfr/L_SUBFR],
				   "FCBindex",FCB_Gain_index[i_subfr/L_SUBFR]);*/
...
void Speech_Decode_Frame( void *st, enum Mode mode, Word16 *parm, enum
	  ...
	  char* feafile,int framenu,int total_frames)
{
   ...
   Word32 i,j;
	...
	if (framenu==1)
	{
		//若已存在文件则先删除再建立
		remove(feafile);
		if((AMRNB_file=fopen(feafile,"a"))==NULL)printf("The files can't be open\n");
	}

   /* Synthesis */
   Decoder_amr( ( ( Speech_Decode_FrameState * ) st )->decoder_amrState, mode,
         parm, frame_type, synth_speech, Az_dec ,
		 LSP_parameter,ACB_integer,ACB_fraction,ACB_Gain,ACB_Gain_index,
		 FCB_pulse,FCB_Gain_index,FCB_Gain);

   for (i = 0; i < 5; i++)
	   fprintf(AMRNB_file, "%d ", LSP_parameter[i]);
   for (i = 0; i < 4; i++)
	   for (j = 0; j < 10; j++)
		   fprintf(AMRNB_file, "%d ", FCB_pulse[i][j]);
   for (i = 0; i < 4; i++)
	   fprintf(AMRNB_file, "%d ", ACB_integer[i]);
   for (i = 0; i < 3; i++) {
	   fprintf(AMRNB_file, "%d ", ACB_fraction[i]);
   }
   fprintf(AMRNB_file, "%d\n", ACB_fraction[3]);

   if(framenu==total_frames)
		fclose(AMRNB_file);
